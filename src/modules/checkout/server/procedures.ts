...
import { PLATFORM_FEE_PERCENTAGE } from "@/modules/home/constant";
...
export const checkoutRouter = createTRPCRouter({
  verify: protectedProcedure
    .mutation(async({ctx}) =>{
      ...
      if(!accountLink.url) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "Falied to create verification link"
        })
      }
  ...
  .mutation(async({ctx, input}) => {
    ...
    products?.docs.map((product) => ({
      quantity: 1,
      price_data: {
        unit_amount: product.price * 100, //Stripe hand;es prices in cents
        currency: "usd",
        product_data: {
          name: product.name,
          metadata: {
